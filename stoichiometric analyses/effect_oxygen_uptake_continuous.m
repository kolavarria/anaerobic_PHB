...
model = changeRxnBounds(model,'EX_Biomass',D,'b');
model = changeObjective(model,'EX_sucrose[e]');
% model = changeObjective(model,'EX_o2[e]');
...
if FBAsolution.f
        ...
        %PHB_per_lean_biomass(i+1)=grams_of_PHB(i+1)/growth_rates(i+1);
...
if FBAsolution.f
        ...
        AARflux(count_2)=(1000/Mw)*FBAsolution.x(findRxnIDs(model,'sintPHB'));
...
figure(1)
% graph_title_1=strcat('dilution rate =',{' '},num2str(D),{' '},'h^{-1}',{' '},'upper row: m_{ATP} =',{' '},num2str(mATP_1*1000/Mw),{' '},'mmol^{ATP}*g_{CDW}*h^{-1}',{' '},'lower row: m_{ATP} =',{' '},num2str(mATP_2*1000/Mw),{' '},'mmol^{ATP}*g_{CDW}*h^{-1}');
% sgt=sgtitle(graph_title_1);
% sgt.FontSize = 8;

subplot(2,2,1);
scatter (qO2_1,qSucrose_1,'k','*');
xlabel('q_{O_2} (mmol*g_{CDW}*h^{-1})');
ylabel('q_{sucrose} (mmol*g_{CDW}*h^{-1})');
x1 = qO2_1;
y1 = qSucrose_1;
x2 = qO2_1;
y2 = percentPHB_1;
