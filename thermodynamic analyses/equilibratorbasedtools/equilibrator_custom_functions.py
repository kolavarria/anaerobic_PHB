...
import math
from equilibrator_api import ComponentContribution, Q_,  ccache
import equilibrator_pathway
from equilibrator_pathway import Pathway
from equilibrator_pathway.thermo_models import PathwayMDFData, PathwayThermoModel
...
class CompoundSettings(object):
	def __init__(self, sd, nd, ad, bs):#,conserved_moieties = []):
		...
		if bs is None:
			bs = Bounds.GetDefaultBounds().Copy()
		self.bounds = bs
	...
	def Copy(self):
		return CompoundSettings(self.acr2kegg_dict.copy(),self.cmpstrs2acr_dict.copy(),self.cmpd2acr_dict.copy(),self.bounds.Copy())#,conserved_moieties=self.conserved_moieties)
	...
	def change_bounds(self,bound_sets):
		...
		for (ca, lb, ub) in bound_sets:
			...
			if compound is None:
				...
				self.bounds.SetBounds(compound, lb, ub)
	...
	def get_bounds(self,cmpd_acronym, bound_type = 'both'):
		if bound_type is 'both':
			return self.bounds.GetBoundTuple(self.acr2cmpd_dict[cmpd_acronym])
		if bound_type is 'lower':
			return self.bounds.GetLowerBound(self.acr2cmpd_dict[cmpd_acronym])
		if bound_type is 'upper':
			return self.bounds.GetUpperBound(self.acr2cmpd_dict[cmpd_acronym])
...
def obtain_compound_settings(filename,custom_bounds = True):
	...
	compounds = list(map(equilibrator_api.ccache.get_compound,keggIDs))
	...
	if custom_bounds:
		...
		for i in range(len(compounds)):
			bds.SetBounds(compounds[i],l_bds[i],u_bds[i])
	else:
		bds = None
...
def create_pathway(filename,cs,pc,absolute_flux = Q_(1.0, 'M/s'),custom_dGs = True):
	(reactions, fluxes,std_dGs) = custom_parse_reactions(filename,cs,absolute_flux = absolute_flux,custom_dGs = custom_dGs)
	pathway = equilibrator_pathway.Pathway(reactions, fluxes,standard_dg_primes=std_dGs,bounds=cs.bounds,config_dict = pc)
	pathway.set_compound_names(CallAcronyms(cs.cmpd2acr_dict))
	return pathway
		...
		for arg in args:
			...
			if(arg != '+' and arg != '<=>' and arg != '=' and not ANumber):
				...
				if keggID == None:
					...
					arg = keggID
			new_rxn += arg + ' '
		new_rxn = PhasedReaction.parse_formula(ccache.get_compound, new_rxn[:-1],rids[i])
...
#Callable class of metabolite acronyms, specific for use as argument in pathway.set_compound_names function
class CallAcronyms:
...
def MDF_analysis(pathway_file_name, cmpd_settings, physiological_context, custom_dGs = True, print_results = True):
...
def MDF_conc_sweep(compound_acronym, conc_range,pathway_file_name, compound_settings, physiological_context, custom_dGs = True):
	cs = compound_settings
	pathway = create_pathway(pathway_file_name,cs,physiological_context,custom_dGs=custom_dGs)
	...
	for i in range(conc_range.size):
		pathway._bounds.SetBounds(compound, Q_(conc_range[i], "M"), Q_(conc_range[i], "M"))
		mdf_result = pathway.calc_mdf()
		mdf_Q = mdf_result.mdf
		mdfs.append(mdf_Q)
	mdfs = np.array([mdf.magnitude for mdf in mdfs])
...
def MDF_double_conc_sweep(cmpd_x_acronym, cmpd_y_acronym, conc_range_x, conc_range_y, pathway_file_name, compound_settings, physiological_context, custom_dGs = True):
	cs = compound_settings
	pathway = create_pathway(pathway_file_name,cs,physiological_context,custom_dGs=custom_dGs)
	...
	for i in range(x):
		for j in range(y):
			pathway._bounds.SetBounds(cmpd_x, Q_(conc_range_x[i], "M"), Q_(conc_range_x[i], "M"))
			pathway._bounds.SetBounds(cmpd_y, Q_(conc_range_y[j], "M"), Q_(conc_range_y[j], "M"))
			mdf_result = pathway.calc_mdf()
...
def MDF_ratio_sweep(num_compound_acronym, den_compound_acronym, ratio_range, total_conc,pathway_file_name, compound_settings, physiological_context, custom_dGs = True):
	...
	for i in range(ratio_range.size):
		pathway._bounds.SetBounds(num_compound, num_conc_range[i], num_conc_range[i])
		pathway._bounds.SetBounds(den_compound, den_conc_range[i], den_conc_range[i])
		mdf_result = pathway.calc_mdf()
		mdf_results.append(mdf_result)
	#mdfs_mag = np.array([mdf.magnitude for mdf in mdfs])
...
def MDF_double_ratio_sweep(ratioX_tuple, ratioY_tuple, pathway_file_name, compound_settings, physiological_context, custom_dGs = True):
	...
	for i in range(x):
		for j in range(y):
			pathway._bounds.SetBounds(n_cmpd_X, n_cr_X[i], n_cr_X[i])
			pathway._bounds.SetBounds(d_cmpd_X, d_cr_X[i], d_cr_X[i])
			pathway._bounds.SetBounds(n_cmpd_Y, n_cr_Y[j], n_cr_Y[j])
			pathway._bounds.SetBounds(d_cmpd_Y, d_cr_Y[j], d_cr_Y[j])
			mdf_result = pathway.calc_mdf()
			results[i,j] = mdf_result
			mdfs[i,j] = mdf_result.mdf.magnitude	
	return mdfs, results
...
def total_enzyme_volume(model, ln_conc):
	...
	enz_conc = list(model.ecf.ECF(ln_conc).flat)
...
def enzyme_partition_distribution(model, ln_conc): 
	partitions = model.ecf.GetEnzymeCostPartitions(ln_conc)
	distrib = []
...
def check_parameters(pathway_file_name, cmpd_settings, physiological_context,abs_flux,tolerance = 0,custom_dGs=True):
	...
	rhs = kcatf_log + model.ecf.D_P_coeff - kcatr_log - model.ecf.D_S_coeff
...
def ECM_analysis(pathway_file_name, cmpd_settings, physiological_context,abs_flux,custom_dGs=True):
	cs = cmpd_settings
	#print(cs.bounds.GetBoundTuple(cs.acr2cmpd_dict['2HB']))
	...
	return ECMmodel(pathway,parameter_df)
...
def ECM_kM_sweep(kM_tuple,pathway_file_name,cs,pc,abs_flux,n_iter = 100,custom_dGs=True):
	...
	tevs = []
	lnCs = []
	new_model = ECMmodel(pathway,p_df)
	lnC = new_model.MDF()
	for value in parameter_range:
		p_df.loc[r_idx,'Value'] = value
		new_model = ECMmodel(pathway,p_df)
		lnC = new_model.ECM(ln_conc0=lnC)
...
def ECM_plot(model,ln_conc,ax,fontsize_ylabel = 12):
	...
	costs = model.ecf.GetEnzymeCostPartitions(ln_conc)
	#print(costs)
	...
	ax.set_xticklabels(xticks, size="medium", rotation=90)
	ax.legend(loc="best", framealpha=0.2) #Activate in case legend inside the plot is wanted
